...
### Installation Guide
Here is an installation tutorial video, which I've uploaded to YouTube (or, you can follow the guide below): https://www.youtube.com/watch?v=nJYyhAxfapg

I would've liked to publish the app on the Play Store to simplify installation, but their developer policy requires all publishers to have legal ownership (or permission from the owner) of all intellectual property featured in their apps. Since my app uses official Pokémon assets (e.g., the images used for the shiny Pokémon and the game icons), I cannot publish it on the Play Store. So, I've instead decided to make the APK file for the app available directly in this GitHub repository. Here's a step-by-step guide on how to install the Pokémon Shiny Tracker app on your Android device:
...
When I release updates to the app, I will upload a new version of the `app-release.apk` file. You will be able to install the latest updates by repeating the installation process with the newest version of the `app-release.apk` file (running it while having an older version of the app installed will update the app while maintaining all of your previous saved data). If you would like to be kept up with updates, please subscribe to my YouTube channel, as I plan to post videos for each update! https://www.youtube.com/channel/UCqeSUHXMz_ygDrB9JQKUBZw
...
# Future Plans
There are two major features I would like to implement in the future. The first feature is a method of exporting/importing saved data. If a user accumulates a lot of saved data on one device but then gets a new device, I don't want to require them to copy every shiny hunt log manually from their old device to their new device. It would be much more convenient if they could export a CSV file with all their saved data, send it to their new device (e.g., email the file to themself), and then import it into the app on their new device. The second feature is a shiny living dex progress checker. Many shiny hunters like to challenge themselves to collect every shiny Pokémon, either in the National Dex or a specific game's Regional Dex. I'd like to add a way to visualize this progress in the app. Likely, it would be in the form of a grid view that displays every Pokémon available in the selected Pokédex (National or Regional). Pokémon that have been obtained (i.e., they have at least one "completed" shiny hunt log) would be highlighted in the grid view, and the rest would be grayed out. There would also be a percentage bar at the top showing how much of the dex has been completed. Of course, the database would need to be heavily modified to specify which Pokémon forms are available in each Pokédex and which Pokémon forms are currently shiny-locked. To make room for these new features in the app, I would probably have to designate the current Home page as an "All Hunts" page and then let the new Home page be a menu with options to View All Hunts, View Shiny Living Dexes, and Export/Import Data. I also plan to periodically provide minor updates if there are any bugs or inefficiencies discovered in the app (such as the loading time of the Pokémon selection dialog). If you find anything that you would like to fix or change, feel free to create an issue/pull request or reach out to me at aaronjcaldwell314@gmail.com. Happy shiny hunting!
